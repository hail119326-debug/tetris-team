...
// 퀴즈를 작은 조각으로 나눠 전송 (큰 데이터가 한 번에 안 가는 문제 방지)
function sendQuizTo(ws) {
  const QZ = globalQuiz || [];
  if (!QZ.length) { ws.send(JSON.stringify({ type: 'quiz', questions: [], timer: globalQuizTimer, reset: true, last: true })); return; }
  const SIZE = 8;
  for (let i = 0; i < QZ.length; i += SIZE) {
    const last = (i + SIZE >= QZ.length);
    ws.send(JSON.stringify({ type: 'quiz', questions: QZ.slice(i, i + SIZE), timer: globalQuizTimer, reset: i === 0, last }));
  }
}
...
function handleMessage(ws, raw) {
  ...
  switch (m.type) {
      ...
      if (isHost(ws)) {
        ...
        if (p) {
          ...
          if (r) {
            ...
            if (globalQuiz.length) sendQuizTo(p);
    ...
    case 'quizreq':          // 학생: 게임 시작 시 현재 퀴즈 당겨오기 (놓침 방지)
      ...
      break;
    case 'setquiz':          // 교사: 퀴즈 문제 등록 (조각으로 받아 이어붙임)
      if (isHost(ws)) {
        const incoming = Array.isArray(m.questions) ? m.questions : [];
        if (typeof m.parts === 'number' && m.parts > 1) {
          if (m.part === 0 || !ws.meta._qbuf) ws.meta._qbuf = [];
          ws.meta._qbuf.push(...incoming);
          if (m.part !== m.parts - 1) break;          // 마지막 조각 전까지 대기
          globalQuiz = ws.meta._qbuf.slice(0, 500);
          ws.meta._qbuf = null;
        } else {
          globalQuiz = incoming.slice(0, 500);          // 단일 전송(작은 경우)
        }
        globalQuizTimer = m.timer !== false;
        for (const p of players()) if (p.meta.room) sendQuizTo(p);
...
function startGame(room, gameType, minutes) {
  ...
  for (const p of playersIn(room.id)) {
    ...
    if (globalQuiz.length) sendQuizTo(p);   // 게임 시작 시 퀴즈 재전송 (조각내서, 놓침 방지)
